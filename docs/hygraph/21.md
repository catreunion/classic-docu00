...
### 1: Collaborate to agree on **data requirements**
...
Backend developers independently design and build their subgraphs.
...
### 5: Composition by the schema registry
...
### 6: The router polls Uplink

The **router** polls Uplink for the latest supergraph schema.
...
The router builds a query plan, with the help of the supergraph schema. It starts with the incoming query's top-level field, looking at the fields in the operation.

It figure out which subgraphs are responsible for resolving each field. The query plan is an ordered list, indicating which smaller requests to send to which appropriate subgraphs.
...
An **entity** is a special **object type** that can resolve its **fields across multiple subgraphs**.

For example, the fields of the **Location** entity are divided between the locations and reviews subgraph. `name`, `description` and `photo` live in the locations subgraph.

The involved subgraphs can contribute fields to and resolve independently.

The subgraph (i.e. `reviews.graphql`) that defines an entity can do one or both of the following:

1. Reference the entity

Review.location has a return type of Location defined in the locations subgraph.
...
A reference resolver is used to return all of the entity fields that a subgraph contributes. VS **Reference resolvers** are used to directly access the entity fields that each subgraph contributes.
...
An entity's primary key is used to identify a unique instance of that entity within a **subgraph**.

An entity can have **more than one** primary key.
...
```js title="A book can be uniquely identified by its ISBN"
const schema = gql`
...
Open up http://localhost:4001 in your browser
...
### Define the entity in reviews subgraphs

1. Add the Location object type, along with the @key directive.

2. Pass in the name of the primary key field as the value of the **fields** property. <-- Set the `id` field as the primary key.

3. Inside the curly braces, add the id field of type non-nullable ID!.

4. A **stub** serves as a **basic representation** of a type that includes just enough information to work with that type in the subgraph. --> Given our reviews subgraph a stub of the Location entity.

5. Inside the @key directive, add one more property called resolvable and set it to false.

6. This property tells the router that the reviews subgraph **doesn't define (as doesn't need) a reference resolver** for this entity.
...
- A reference resolver is used to return all of the entity fields that a subgraph contributes.
...
Location.name can only be resolved by the locations subgraph. The router needs more information **from the reviews subgraph** : the **entity representation** for each review's corresponding Location object.

The router needs the **location's typename** and its **primary key** to track a specific object between subgraphs.

The router adds another operation to its query plan to request the location's name from the locations subgraph.

The router builds the request using the \_entities field.This field takes in an argument called **representations**. The router sends this request to the location's subgraph.

To resolve the \_entities field, the locations subgraph uses its reference resolver to return all the entity fields that this subgraph contributes.

The locations subgraph looks at the \_\_typename value of each reference object to determine which entity's reference resolver to use.
